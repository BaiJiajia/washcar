...
export default {
        ...
        myAddressSlots: [
            {
                flex: 1,
                defaultIndex: 1,
                values: Object.keys(cityList),  //省份数组
                className: 'slot1',
                textAlign: 'center'
            }, {
                divider: true,
  ...
  methods:{
    ...
    todetail(shop){
      this.$router.push({path: '/about',query:{...shop, distance: this.getFlatternDistance(this.point.lat, this.point.lng, shop.lat, shop.lng).toFixed(1)}})
    },
    onValuesChange(picker, values) {
        if(cityList[values[0]]){  //这个判断类似于v-if的效果（可以不加，但是vue会报错，很不爽）
            picker.setSlotValues(1,Object.keys(cityList[values[0]])); // Object.keys()会返回一个数组，当前省的数组
            picker.setSlotValues(2,cityList[values[0]][values[1]]); // 区/县数据就是一个数组
                  ...
                  if(Object.keys(this.cityList)[i] === this.province) {
                      ...
                      for(let j in Object.keys(this.cityList[this.province])) {
                          if(Object.keys(this.cityList[this.province])[j] === this.city) {
                              ...
                              for(let k in this.cityList[this.province][this.city]) {
                                  if(this.cityList[this.province][this.city][k] === this.district) {
